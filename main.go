...
func init() {
	...
	LightGopherBlue, err = hexToColor("96D7FF")
	if err != nil {
		log.Fatal(err)
	}
	DarkGopherBlue, err = hexToColor("94D4FB")
	if err != nil {
		log.Fatal(err)
	}
}
...
func main() {
	...
	// Open the gopherbutt gif
	gifimage, err := os.Open("gophersbutt.gif")
	if err != nil {
		log.Fatal(err)
	}
	defer gifimage.Close()

	// Decode the gif so we can edit it
	gopher, err := gif.DecodeAll(gifimage)
	if err != nil {
		log.Fatal(err)
	}
	...
	// Save it out!
	o, _ := os.OpenFile("gophersbutt-party.gif", os.O_WRONLY|os.O_CREATE|os.O_TRUNC, 0600)
	defer o.Close()
